...
2. **Run the schema.** Open your project's **SQL Editor**, paste in the contents of `supabase-schema.sql` from this repo, and run it. This creates the `profiles`, `progress`, and `friendships` tables with Row Level Security policies, plus a `leaderboard_entries` view.
